...
public class UserServiceImpl implements UserService {
    ...
    public Response getUserInfoAndOrderHistory() {
        ...
        // Explicitly fetch user with order items to ensure latest data
        user = userRepo.findById(user.getId()).orElseThrow(() -> new UsernameNotFoundException("User not found"));
        // If using Hibernate, initialize order items (optional, if lazy)
        if (user.getOrderItemList() != null) {
            user.getOrderItemList().size(); // Force initialization
        }
